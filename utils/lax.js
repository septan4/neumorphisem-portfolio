...
;(() => {
  const lax = (() => {
    ...
    const transformFns = {
      'data-lax-opacity': (style, v) => {
        style.opacity = v
      },
      'data-lax-translate-abs': (style, v) => {
        style.transform += ` translate(${v}px, ${v}px)`
      },
      'data-lax-translate-x-abs': (style, v) => {
        style.transform += ` translateX(${v}px)`
      },
      'data-lax-translate-y-sbs': (style, v) => {
        style.transform += ` translateY(${v}px)`
      },
      'data-lax-translate-3d-abs': (style, v) => {
        style.transform += ` translate3d(0px,${v}px,0px)`
      },
      'data-lax-translate-rel': (style, v) => {
        style.transform += ` translate(${v}%, ${v}%)`
      },
      'data-lax-translate-x-rel': (style, v) => {
        style.transform += ` translateX(${v}%)`
      },
      'data-lax-translate-y-rel': (style, v) => {
        style.transform += ` translateY(${v}%)`
      },
      'data-lax-translate-3d-rel': (style, v) => {
        style.transform += ` translate3d(${v}%,${v}%,0px)`
      },
      'data-lax-border-radius': (style, v) => {
        style.borderRadius += ` ${v}px`
      },
      'data-lax-scale': (style, v) => {
        style.transform += ` scale(${v})`
      },
      'data-lax-scale-x': (style, v) => {
        style.transform += ` scaleX(${v})`
      },
      'data-lax-scale-y': (style, v) => {
        style.transform += ` scaleY(${v})`
      },
      'data-lax-skew': (style, v) => {
        style.transform += ` skew(${v}deg, ${v}deg)`
      },
      'data-lax-skew-x': (style, v) => {
        style.transform += ` skewX(${v}deg)`
      },
      'data-lax-skew-y': (style, v) => {
        style.transform += ` skewY(${v}deg)`
      },
      'data-lax-rotate': (style, v) => {
        style.transform += ` rotate(${v}deg)`
      },
      'data-lax-rotate-x': (style, v) => {
        style.transform += ` rotateX(${v}deg)`
      },
      'data-lax-rotate-y': (style, v) => {
        style.transform += ` rotateY(${v}deg)`
      },
      'data-lax-brightness': (style, v) => {
        style.filter += ` brightness(${v}%)`
      },
      'data-lax-contrast': (style, v) => {
        style.filter += ` contrast(${v}%)`
      },
      'data-lax-hue-rotate': (style, v) => {
        style.filter += ` hue-rotate(${v}deg)`
      },
      'data-lax-blur': (style, v) => {
        style.filter += ` blur(${v}px)`
      },
      'data-lax-invert': (style, v) => {
        style.filter += `  invert(${v}%)`
      },
      'data-lax-saturate': (style, v) => {
        style.filter += `  saturate(${v}%)`
      },
      'data-lax-grayscale': (style, v) => {
        style.filter += `  grayscale(${v}%)`
      },
      'data-lax-bg-pos': (style, v) => {
        style.backgroundPosition = `${v}px ${v}px`
      },
      'data-lax-bg-pos-x': (style, v) => {
        style.backgroundPositionX = `${v}px`
      },
      'data-lax-bg-pos-y': (style, v) => {
        style.backgroundPositionY = `${v}px`
      }
    }
    ...
    lax.presets = {
      ...
      slalom: (v = 50) => {
        return {
          'data-lax-translate-x': `vh ${v}, (vh*0.8) ${-v}, (vh*0.6) ${v}, (vh*0.4) ${-v}, (vh*0.2) ${v}, (vh*0) ${-v}, (-elh) ${v}`
        }
      },
      crazy: v => {
      ...
      zoomInOut: (v = 0.2) => {
        return {
          'data-lax-scale': `(vh) ${v}, (vh*0.8) 1, (vh*0.2) 1, -elh ${v}`
        }
      },
      zoomIn: (v = 0.2) => {
    ...
    lax.calcTransforms = o => {
      ...
      const spriteData =
        el.attributes['data-lax-sprite-data'] &&
        el.attributes['data-lax-sprite-data'].value
      if (spriteData) {
        ...
        const spriteImage =
          el.attributes['data-lax-sprite-image'] &&
          el.attributes['data-lax-sprite-image'].value
        if (spriteImage) {
    ...
    lax.updateElement = o => {
      ...
      for (let i in transforms) {
        const transformData =
          transforms[i][currentBreakpoint] || transforms[i]['default']
      ...
      if (spriteData) {
        ...
        style.backgroundPosition = `-${framePosX * frameW}px -${
          framePosY * frameH
        }px`
      }
